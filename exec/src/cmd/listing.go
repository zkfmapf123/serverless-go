...
var listingCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(list) == 0 {
			fmt.Println("Not Exists Lambda")
			os.Exit(0)
		}

		filesystem.PrintTable[aws.LambdaInfo](list, []string{"Name", "Desc", "Env Count", "Size", "Last Updated"},
			func(k string, v aws.LambdaInfo) []string {
				envString, sizeString := strconv.Itoa(v.Env), strconv.Itoa(int(v.Size))
